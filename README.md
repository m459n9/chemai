...
```
.
├── chemai_final.ipynb         # финальный пайплайн (EDA + решение, 48 ячеек)
├── pipeline_search.ipynb      # research-тетрадь с grid-search и Optuna
├── data/
...
(не аугментированном) train-наборе, в `pipeline_search.ipynb`, секция «Optuna
на лучшей конфигурации». Тюнить совместно с mixup-аугментацией некорректно —
параметры подгоняются под синтетику, а не под целевое распределение.
...
Все эксперименты задокументированы в `pipeline_search.ipynb`.

## Чек-лист воспроизводимости и честности

- [x] Все seeds зафиксированы (`SEED = 42`).
- [x] Никаких внешних данных (только разрешённые `train.csv` / `test.csv`).
- [x] Нет утечек тестовых таргетов в обучение (OOF — стандартный 5-fold,
      mixup применяется только к тренировочным фолдам).
- [x] Нет ручной подгонки под public LB (одна финальная конфигурация,
      выбранная по OOF на дедуплицированном train).
- [x] Submission генерируется одной командой `Run All` (chemai_final.ipynb).
